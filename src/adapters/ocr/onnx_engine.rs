...
use ort::ep::{CUDAExecutionProvider, TensorRTExecutionProvider, DirectMLExecutionProvider};
use std::path::Path;
...
impl OnnxEngine {
    ...
    pub fn create_session<P: AsRef<Path>>(model_path: P, gpu_backend: GpuBackend) -> Result<Session> {
        ...
        let mut builder = builder.with_execution_providers(execution_providers)
            .map_err(|e| anyhow::anyhow!("ORT EP Error: {}", e))?;

        // Log which GPU backend is being used for verification
        tracing::info!("Creating ONNX session for {:?} with GPU backend: {:?}",
            model_path.as_ref(), gpu_backend);
